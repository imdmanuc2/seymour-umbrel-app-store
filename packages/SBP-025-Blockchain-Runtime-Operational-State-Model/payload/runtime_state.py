...
def normalize_runtime_state(runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if not installed:
        ...
        state, reason = "stopped", "Runtime container is not running."
    elif container_health == "starting":
        state, reason = "starting", "Runtime is starting or RPC is warming up."
    elif rpc_reachable and rpc_healthy and ibd is True:
        state, reason = "syncing", "Runtime RPC is healthy and initial block download is active."
    elif rpc_reachable and rpc_healthy and rpc_status == "rpc-slow":
        state, reason = "syncing", "Runtime RPC is healthy while detailed synchronization telemetry is slow."
    elif rpc_reachable and rpc_healthy:
        state, reason = "healthy", "Runtime RPC is healthy."
    else:
        state, reason = "degraded", "Runtime is running but one or more health signals are degraded."
