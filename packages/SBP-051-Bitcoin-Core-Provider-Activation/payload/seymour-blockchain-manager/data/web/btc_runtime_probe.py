...
from btc_rpc_probe import probe as probe_btc_rpc
from runtime_state import normalize_runtime_state

DOCKER_SOCKET=Path(os.environ.get("DOCKER_SOCKET","/var/run/docker.sock"))
BTC_NODE_CONTAINER=os.environ.get("BTC_NODE_CONTAINER","seymour-bitcoin-node_node_1")
BTC_HEALTH_URL=os.environ.get("BTC_HEALTH_URL","http://status:8080/api/health")
BTC_STATUS_URL=os.environ.get("BTC_STATUS_URL","http://status:8080/api/status")
...
def docker_container_inspect(name: str=BTC_NODE_CONTAINER)->dict[str,Any]:
...
def probe()->dict[str,Any]:
 ...
 legacy_health=http_json(BTC_HEALTH_URL, timeout=8)
 legacy_status=http_json(BTC_STATUS_URL, timeout=25)
 rpc_probe=probe_btc_rpc()
 ...
 result = {"providerId":"bitcoin-mainnet","appId":"seymour-bitcoin-node","installed":installed,"running":running,"lifecycleStatus":lifecycle,"container":container,"rpc":{"reachable":rpc,"probe":rpc_probe,"health":legacy_health,"status":legacy_status}}
