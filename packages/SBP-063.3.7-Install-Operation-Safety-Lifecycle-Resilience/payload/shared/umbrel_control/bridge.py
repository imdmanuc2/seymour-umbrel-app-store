...
from .http_client import UmbrelHttpClient
        ...
        try:
            ...
            if app_id is not None and action in {"start", "restart", "stop", "install"}:
                try:
                    ...
                    state_matches = (
                        _state_matches_action(action, current_state)
                        or (
                            action == "install"
                            and str(current_state or "").strip().lower()
                            in {"ready", "running"}
                        )
                    )
                    if state_matches:
